...
def doMatrixMinimization(self, extraPatients, progress):
    ...
    if self.options.fixA.get(): aList = aList[0:1]
    if self.options.fixB.get(): bList = bList[0:1]
    if self.options.fixN.get(): nList = nList[0:1]
    if self.options.fixM.get(): mList = mList[0:1]
    if self.options.fixTD50.get(): TD50List = TD50List[0:1]
...
def doGradientOptimization(self, extraPatients, progress):  
    ...
    elif self.options.optimizationMetric.get() == "LS" and self.options.NTCPcalculation.get() == "LKB": fun = funLKBLS
    elif self.options.optimizationMetric.get() == "LS" and self.options.NTCPcalculation.get() == "Logit": fun = funLogitLS
    else:
