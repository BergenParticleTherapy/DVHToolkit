...
def calculateDVHCommand(self):
	...
	self.calculateSIndexContainer = Frame(self.inputContainer)
	self.calculateSIndexContainer.pack(anchor=W)
	...
	self.dvhCheckVarCalculateSIndex = IntVar(value=0)
	...
	Checkbutton(self.calculateSIndexContainer, text="Calculate the S-index", variable=self.dvhCheckVarCalculateSIndex).pack(anchor=W)
	Tooltip(self.calculateSIndexContainer, text="Calculate the S-index, or the standard deviation of the PTV width using the dDVH.",
	      	wraplength=self.wraplength)
